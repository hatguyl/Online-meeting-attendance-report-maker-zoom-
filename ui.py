...
class App(TkinterDnD.Tk):
    def __init__(self):
        ...
        # | WINDOW CONFIG  |
        # +----------------+
        self.title("Attendance Report tool")
        ...
        logo_img = Image.open(resource_path("logo.png")).resize((220, 80), Image.LANCZOS)
        self.logo = ctk.CTkImage(light_image=logo_img, dark_image=logo_img, size=(220, 80))

        icon_img = Image.open(resource_path("logo.png")).resize((64, 64), Image.LANCZOS)
        self.iconphoto(False, ImageTk.PhotoImage(icon_img))

        ctk.CTkLabel(self.card, image=self.logo, text="").pack(pady=(15, 5))

        # +----------------+
        # | CSV DROP FILE  |
        # +----------------+
        self.drop = ctk.CTkFrame(self.card, height=150, corner_radius=18, fg_color=self.FIELD)
        self.drop.pack(padx=20, pady=15, fill="x")
        ...
        # | BATCH SELECT   |
        # +----------------+
        ctk.CTkLabel(self.card, text="Batch Timimg", text_color=self.MUTED).pack(pady=(10, 5))
        ...
        # | ATT INPUTS     |
        # +----------------+
        self.att_entry = self.labeled_entry("Attendance Due (minutes)")
        self.abs_entry = self.labeled_entry("Absent Due (minutes)")
        ...
        time_row.pack(fill="x", padx=30, pady=4)

        self.time_entry = ctk.CTkEntry(
            time_row, textvariable=self.late_time,
            height=36, fg_color=self.FIELD,
            border_color="#555555"
        )
        ...
        # +----------------+
        ctk.CTkButton(
            self.card, text="GENERATE REPORT",
            height=42, fg_color=self.ACCENT,
            hover_color="#ffd95a", text_color="#000000",
            command=self.run
        ).pack(pady=22, fill="x", padx=30)
